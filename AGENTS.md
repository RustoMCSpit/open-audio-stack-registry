...
If check finds a match that is the same version code, **stop**. If the match is a newer version, **continue**. Comment on the issue to let the submitter know the package is already in the registry and include the existing registry URL:
